    ...
    <div class="main-container">
            ...
            <form action="{{ route('submit.exam') }}" method="POST" id="examForm">
                ...
                @if (isset($data))
                    <div class="questions">
                        ...
                        @foreach ($data['data']['questions'] as $q)
                            <div class="question-card" data-index="{{ $loop->index }}"
                                data-competencies='@json($q["competencies"])' data-q-type='{{ $q["type"] }}'
                                data-choices-equivalent='@json($q["choice_equivalent"] ?? $q["choices_equivalent"] ?? null)'>
                                ...
                                <div class="question-card-a">
                                    ...
                                    <input type="hidden" data-category="category[{{ $loop->index }}]"
                                    ...
                                    <ul class="choices-holder">
                                        @foreach ($q['choices'] as $letter => $text)
                                            <li class="choices">
                                                <label>
                                                    <input class="radio" type="radio" name="answer[{{ $loop->parent->index }}]"
                                                        value="{{ $letter }}">
                    ...
                    <div class="buttons" id="controls">
                        <button type="button" class="btn prev-btn" id="prevBtn"><i class="icon icon-arrow-left"></i>
                            Previous</button>
                        <button type="button" class="btn next-btn" id="nextBtn">Next Question<i
        ...
        <div class="right">
            ...
            <div class="q-nav-container">
                <div class="q-nav-header"><i class="icon icon-grid"></i>
                    <h3>Question Navigator</h3>
                </div>
                <div class="question-navigator">
                    @foreach ($data['data']['questions'] as $q)
                        <button type="button" class="nav-q-btn"
                            data-index="{{ $loop->index }}">{{ $loop->index + 1 }}</button>
                    @endforeach
                </div>
                <div class="q-nav-info">
                    <div class="q-nav-ind"><span class="indicator"></span>
                        <h3>Current</h3>
                    </div>
                    <div class="q-nav-ind"><span class="indicator"></span>
                        <h3>Answered</h3>
        ...
        <div class="alert-bg feedback-form hidden">
            <div class="modal">
                ...
                <form id="feedback-form" class="feedback-form">
                    ...
                    <input type="hidden" id="email" name="email" placeholder="Your Email" required value="{{ $useremail }}">
                    <input type="hidden" id="title" name="title" placeholder="Title" required value="Feedback">
                    <textarea type="text" id="message" class="feedback-input" name="message" placeholder="Enter your feedback here..."></textarea>
                    <div class="alert-button-container">
                        <button type="button" class="btn btn-secondary"><i class="icon-arrow-right"></i> Cancel</button>
                        <button type="button" class="btn btn-middle">Skip <i class="icon-arrow-right"></i></button>
                        <button type="button" class="btn btn-primary">Submit <i class="icon-send"></i></button>
                    </div>
                </form>
            </div>
        </div>
